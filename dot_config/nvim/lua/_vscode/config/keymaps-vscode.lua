...
map("n", "<c-f>", "<C-d><C-d>", opts)
map("x", "<c-f>", "<C-d><C-d>", opts)
-- map("n", "<c-p>", "<C-u><C-u>", opts)
-- map("x", "<c-p>", "<C-u><C-u>", opts)
...
map('n', ',,', ',', opts)
map('n', ';;', ';', opts)
map('n', '<leader><C-l>', '<C-l>', opts)

-- Search without moving cursor
map('n', '*', 'm`*``', { desc = "Search word under cursor without moving" })

-- Clear search highlighting
map('n', '<Esc><Esc>', ':nohlsearch<CR><Esc>', { desc = "Clear search highlight" })

-- Japanese input mode helpers
map('n', 'い', 'i', { desc = "Insert mode (Japanese い → i)" })
map('n', 'あ', 'a', { desc = "Append mode (Japanese あ → a)" })

-- Text selection and yanking
map('n', 'vaa', 'gg<S-v>G', { desc = "Select all" })
map('n', 'yaa', 'mzggyG`z', { desc = "Yank all" })

-- Visual mode improvements
map('x', '<', '<gv', { desc = "Decrease indent and reselect" })
map('x', '>', '>gv', { desc = "Increase indent and reselect" })
...
map('i', '<c-o><c-l>', '<c-x><c-l>', { desc = "Line completion" })
map('i', '<c-o><c-p>', '<c-x><c-p>', { desc = "Keyword completion" })
map('i', '<c-o><c-n>', '<c-x><c-n>', { desc = "Keyword completion" })
map('i', '<c-o><c-k>', '<c-x><c-k>', { desc = "Dictionary completion" })
map('i', '<c-o><c-t>', '<c-x><c-t>', { desc = "Thesaurus completion" })
map('i', '<c-o><c-i>', '<c-x><c-i>', { desc = "Include file completion" })
map('i', '<c-o><c-]>', '<c-x><c-]>', { desc = "Tag completion" })
map('i', '<c-o><c-f>', '<c-x><c-f>', { desc = "File path completion" })
map('i', '<c-o><c-d>', '<c-x><c-d>', { desc = "Definition completion" })
map('i', '<c-o><c-v>', '<c-x><c-v>', { desc = "Vim command completion" })
map('i', '<c-o><c-u>', '<c-x><c-u>', { desc = "User-defined completion" })
map('i', '<c-o><c-o>', '<c-x><c-o>', { desc = "Omni completion" })
map('i', '<c-o><c-s>', '<c-x>s', { desc = "Spell completion" })
...
-- File path yanking
map('n', 'yn', function() vim.fn.setreg('+', vim.fn.expand('%:t')) end, { desc = "Yank filename" })
map('n', 'yp', function()
...
map('n', 'yR', ':YankRelativePath<CR>', { desc = "Yank relative path from input" })
...
map('n', 's', '<Nop>', opts) -- Disable 's' to use as prefix
...
-- カーソル位置の折りたたみ/展開
map('n', 'zc', function() vscode.action('editor.fold') end, { silent = true })
map('n', 'zo', function() vscode.action('editor.unfold') end, { silent = true })

-- すべて折りたたむ/展開
map('n', 'zM', function() vscode.action('editor.foldAll') end, { silent = true })
map('n', 'zR', function() vscode.action('editor.unfoldAll') end, { silent = true })

-- （お好み）VS Code の「レベル折りたたみ」コマンド
-- zm / zr を VS Code 流にエミュレートしたいなら、foldLevelN を使う
map('n', 'z1', function() vscode.action('editor.foldLevel1') end, { silent = true })
map('n', 'z2', function() vscode.action('editor.foldLevel2') end, { silent = true })
-- 必要に応じて z3..z9 も同様に
